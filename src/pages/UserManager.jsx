...
const UserManager = () => {
  ...
  const getRoleBadge = (role) => {
    switch (role) {
      ...
      default:
        return <span className="px-2 py-1 bg-gray-200 text-gray-700 text-[10px] font-black rounded uppercase tracking-widest">Operator</span>;
    }
  };
  ...
  return (
    <div className="max-w-5xl mx-auto pb-24 animate-fade-in relative">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-2xl font-black text-gray-900 flex items-center gap-2"><Shield className="text-brand"/> Gestão de Acessos</h1>
          <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">Gerencie quem acessa o seu Nexus ITAM</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="md:col-span-2 space-y-6">
          <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm">
            <h3 className="text-sm font-black text-gray-900 mb-6 uppercase tracking-wider flex items-center gap-2">Membros Ativos <span className="bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full text-[10px]">{users.length}</span></h3>
              ...
              {users.map(user => (
                <div key={user.id} className="flex items-center justify-between p-4 border border-gray-100 rounded-2xl hover:bg-gray-50 transition-colors">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-bold text-gray-900">{user.name || 'Usuário Nexus'}</p>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    {user.id !== currentUser.uid && (
                      <select 
                        ...
                        className="text-[10px] font-bold border border-gray-200 rounded p-1 bg-white outline-none cursor-pointer"
                      >
          ...
          {invites.length > 0 && (
            <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm">
              <h3 className="text-sm font-black text-gray-900 mb-6 uppercase tracking-wider text-gray-500">Convites Pendentes</h3>
              <div className="space-y-3">
                {invites.map(invite => (
                  <div key={invite.id} className="flex items-center justify-between p-3 border border-gray-100 rounded-xl bg-gray-50">
                    <div>
                      <p className="text-xs font-bold text-gray-700 flex items-center gap-1"><Mail size={12}/> {invite.email}</p>
                      <p className="text-[10px] text-gray-400 mt-1 uppercase">Como: {invite.role}</p>
                    </div>
        ...
        <div className="md:col-span-1">
            ...
            <form onSubmit={handleInvite} className="space-y-4 relative z-10">
              <div>
                ...
                <input 
                  ...
                  className="w-full p-3 rounded-xl bg-white/10 border border-white/20 focus:bg-white focus:text-black focus:outline-none transition-all placeholder:text-white/30 text-sm font-bold text-white"
                />
              </div>
              ...
              <div>
                ...
                <select 
                  ...
                  className="w-full p-3 rounded-xl bg-white/10 border border-white/20 focus:bg-white focus:text-black focus:outline-none transition-all text-sm font-bold text-white [&>option]:text-black"
                >
              ...
              <div className="pt-2">
                <button type="submit" disabled={isInviting} className="w-full py-3 bg-white text-brand font-black rounded-xl hover:bg-gray-100 transition-colors shadow-md text-xs uppercase tracking-widest flex items-center justify-center gap-2">
          ...
          <div className="mt-6 bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
            <h4 className="text-xs font-black text-gray-900 uppercase tracking-widest mb-3">Entendendo os Papéis</h4>
            <ul className="space-y-3">
              <li className="flex gap-2">
                <ShieldCheck size={16} className="text-black shrink-0"/>
                <div>
                  <p className="text-xs font-bold text-gray-900">Owner</p>
                  <p className="text-[10px] text-gray-500">Acesso total. O único que pode acessar as configurações, cores da marca e gestão de usuários.</p>
                </div>
              </li>
              <li className="flex gap-2">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-gray-500">Pode criar, editar e excluir ativos. Gerencia funcionários e licenças.</p>
                </div>
              </li>
              <li className="flex gap-2">
                <CheckCircle2 size={16} className="text-gray-400 shrink-0"/>
                <div>
                  <p className="text-xs font-bold text-gray-900">Operator</p>
                  <p className="text-[10px] text-gray-500">Nível mais básico. Não pode excluir ativos ou gerenciar contratos.</p>
                </div>
              </li>
            </ul>
          </div>
