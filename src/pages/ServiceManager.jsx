    ...
    <div className="p-8 max-w-[1600px] mx-auto">
      ...
      <div className="bg-neutral-900 text-white p-6 rounded-2xl shadow-lg mb-8 flex justify-between items-center relative overflow-hidden">
          <div className="relative z-10">
              <p className="text-gray-400 dark:text-gray-500 text-xs font-bold uppercase mb-1">Custo Mensal Recorrente</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {contracts.map(contract => (
            <div key={contract.id} className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-gray-200 dark:border-slate-600 shadow-sm hover:shadow-md transition-all group relative">
                ...
                <div className="flex items-center gap-3 mb-4">
                    <div className="p-3 bg-gray-50 dark:bg-slate-900 rounded-xl text-brand font-bold border border-gray-100 dark:border-slate-700">
                ...
                <div className="space-y-3 text-sm text-gray-600">
                    <div className="flex justify-between border-b border-gray-50 pb-2">
                        ...
                        <span className="font-bold text-gray-900 dark:text-white">R$ {parseFloat(contract.monthlyCost).toLocaleString('pt-BR')}</span>
                    </div>
                    <div className="flex justify-between border-b border-gray-50 pb-2">
                        <span className="flex items-center gap-2"><Phone size={14}/> Suporte</span>
                        <span className="font-mono text-gray-800 dark:text-gray-100">{contract.supportPhone || "---"}</span>
                    </div>
                    <div className="flex justify-between border-b border-gray-50 pb-2">
                        <span className="flex items-center gap-2"><Calendar size={14}/> Renovação</span>
                        <span className={`font-bold ${new Date(contract.renewalDate) < new Date() ? 'text-red-500' : 'text-gray-800 dark:text-gray-100'}`}>
