...
const TaskManager = () => {
  ...
  const KANBAN_COLS = [
      { id: 'A Fazer', label: 'A Fazer', color: 'bg-gray-100 border-gray-200 dark:border-slate-600' },
  ...
  const TaskCard = ({ task }) => (
      <div className={`bg-white dark:bg-slate-800 p-4 rounded-xl border shadow-sm group hover:shadow-md transition-all ${task.priority === 'Alta' && task.status !== 'Concluído' ? 'border-l-4 border-l-red-500 border-gray-200 dark:border-slate-600' : 'border-gray-200 dark:border-slate-600'}`}>
          <div className="flex justify-between items-start mb-2">
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded border uppercase ${task.priority === 'Alta' ? 'bg-red-100 text-red-700 border-red-200' : 'bg-gray-100 text-gray-600 border-gray-200 dark:border-slate-600'}`}>
                  {task.priority || 'Média'}
              </span>
              <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => { setEditingTask(task); setIsEditOpen(true); }} className="p-1 hover:bg-gray-100 rounded text-gray-400 dark:text-gray-500 hover:text-blue-600"><Edit size={14}/></button>
                  <button onClick={() => handleDelete(task.id)} className="p-1 hover:bg-gray-100 rounded text-gray-400 dark:text-gray-500 hover:text-red-600"><Trash2 size={14}/></button>
              </div>
          </div>
          
          <p className={`font-bold text-sm mb-3 ${task.status === 'Concluído' ? 'text-gray-400 dark:text-gray-500 line-through' : 'text-gray-800 dark:text-gray-100'}`}>{task.title}</p>
    ...
    <div className="p-4 md:p-8 max-w-[1600px] mx-auto pb-24 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end mb-8 gap-4">
          ...
          <div className="flex items-center gap-2 bg-white dark:bg-slate-800 p-1 rounded-xl border border-gray-200 dark:border-slate-600 shadow-sm">
             <button onClick={() => setViewMode('list')} className={`p-2 rounded-lg transition-all ${viewMode === 'list' ? 'bg-black text-white shadow' : 'text-gray-400 dark:text-gray-500 hover:text-black'}`}><List size={20}/></button>
      ...
      <form onSubmit={handleQuickAdd} className="mb-8 relative group max-w-2xl mx-auto">
          <input 
            ...
            className="w-full p-4 pl-12 bg-white dark:bg-slate-800 border-2 border-gray-200 dark:border-slate-600 rounded-2xl outline-none focus:border-black font-medium shadow-sm transition-all group-hover:border-gray-300"
          />
          <Plus className="absolute left-4 top-4.5 text-gray-400 dark:text-gray-500 group-hover:text-black transition-colors" size={24}/>
      ...
      {loading ? <p className="text-center text-gray-400 dark:text-gray-500 py-10">Carregando...</p> : (
          viewMode === 'kanban' ? (
                      ...
                      <div key={col.id} className="min-w-[280px] w-full flex-1 flex flex-col bg-gray-50/80 rounded-2xl border border-gray-200 dark:border-slate-600">
                          ...
                          <div className="p-2 space-y-2 overflow-y-auto max-h-[60vh] custom-scrollbar">
                              ...
                              {getTasksByStatus(col.id).length === 0 && (
                                  <div className="h-24 flex items-center justify-center border-2 border-dashed border-gray-200 dark:border-slate-600 rounded-xl m-2 opactiy-50">
                                      <span className="text-xs text-gray-400 dark:text-gray-500 font-bold uppercase">Vazio</span>
                                  </div>
                              )}
                          </div>
                      </div>
              ...
              <div className="space-y-3 max-w-4xl mx-auto">
                  {tasks.length === 0 ? (
                      <div className="text-center py-16 bg-gray-50 dark:bg-slate-900 rounded-2xl border border-dashed border-gray-200 dark:border-slate-600">
                          <p className="text-gray-400 dark:text-gray-500 text-sm font-medium">Nenhuma tarefa encontrada.</p>
                      </div>
                   ) : (
                      tasks.map(task => (
                          <div key={task.id} className={`group flex items-center gap-4 bg-white dark:bg-slate-800 p-4 rounded-xl border transition-all hover:shadow-md ${task.status === 'Concluído' ? 'opacity-60' : ''}`}>
                              ...
                              <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                  <button onClick={() => { setEditingTask(task); setIsEditOpen(true); }} className="p-2 text-gray-400 dark:text-gray-500 hover:text-blue-600 hover:bg-blue-50 rounded-lg"><Edit size={16}/></button>
                                  <button onClick={() => handleDelete(task.id)} className="p-2 text-gray-400 dark:text-gray-500 hover:text-red-600 hover:bg-red-50 rounded-lg"><Trash2 size={16}/></button>
                              </div>
                          </div>
                      ))
                   )}
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
              <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md p-6 animate-in zoom-in-95">
                  <div className="flex justify-between items-center mb-4">
                      <h2 className="text-xl font-black text-gray-900 dark:text-white flex items-center gap-2"><Edit size={20}/> Editar Tarefa</h2>
                      <button onClick={() => setIsEditOpen(false)}><X size={20} className="text-gray-400 dark:text-gray-500 hover:text-black"/></button>
                  </div>
